        ...
        $('#btn_envoyer').click(function(){
            if($('#expediteur').val()=='' || $('#message').val()==''){
              ...
              $.ajax({
                      ...
                      dataType:'html', 
                      data:{message:$('#message').val(), conversation:$('#ID_Conversation').val(), expediteur:$('#expediteur').val(), nbr_sms:$('#nbr_sms').val(), temp_envoi:$('#temp_envoi').val()},
                      success:function(ret){
                           waitingDialog.hide();
                           if(ret==1){
                              alertify.alert('SMS', 'Transmission effectuée');
                              alertify.success('Transmission effectuée');
                           }else if(ret==2){
                             alertify.alert('SMS', 'Messages planifiés');
                             alertify.success('Messages planifiés');
                           }else{
    ...
    $('#message').keyup(function(){
        ...
        $.ajax({
          ...
          success:function(ret){
            nbrSMS=ret;
            if(ret==0){
                $('#ptext').text('Vous avez dépassé le nombre maximun des caractères (1280 pour le texte simple et 560 pour Unicode) qui vaut 8 SMS.');
                $('#mtext').slideDown('slow');
                $('#nbr_sms').val('max');
