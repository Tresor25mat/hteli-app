...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed" style="overflow-x: hidden;">
              ...
              <div class="col-12">
                <div class="row" style="border-bottom: 1px solid #EEEEEE; padding-bottom: 20px">
                            ...
                            <div class="col-sm-12">
                                <select name="pays" class="form-control" id="pays" <?php if($_SESSION['user_eteelo_app']['ID_Statut']!=1){echo 'disabled'; } ?>>
                                    ...
                                    <?php while ($pays = $pay->fetch()) { ?>
                                        <option value="<?php echo ($pays['ID_Pays']) ?>" <?php if ($pays['ID_Pays']==$_SESSION['user_eteelo_app']['ID_Pays']) { echo 'selected';}else if(isset($_GET['Pays']) && $_GET['Pays']!='' && $pays['ID_Pays']==$_GET['Pays']){echo 'selected'; } ?>><?php echo (stripslashes($pays['Design_Pays'])) ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                          </div>
                    ...
                    <div class="col-12 col-md-auto ms-auto d-print-none" style="margin-top: 18px">
                        <div class="btn-list">
                        <a href="#" id="btn_ajouter" class="btn btn-primary d-sm-inline-block" title="Nouvelle province">
    ...
    $(document).ready(function() {
        $('#iframe').attr('src', "table_province.php?Pays="+$('#pays').val());
    });
  ...
  $(function() {
    ...
    $('#btn_ajouter').click(function(e){
      e.preventDefault();
      window.location.replace('ajouter_province.php?Pays='+$('#pays').val());
    })
