...
<html lang="fr-FR">
  <head>
    ...
    <title>Monthly Tower Inspection | <?php echo $app_infos['Design_App']; ?></title>
    <!-- CSS files -->
    <!-- DataTables CSS -->
...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed" style="overflow-x: hidden;">
            ...
            <div class="row g-2 align-items-center">
              ...
              <div class="col-12">
                <div class="row" style="border-bottom: 1px solid #EEEEEE; padding-bottom: 20px">
                    ...
                    <div class="col-md-<?php if($_SESSION['user_eteelo_app']['ID_Statut']==3){ echo '2';}else{echo '1';} ?>">
                      <div class="form-group ">
                        <label for="TowerType" class="control-label col-lg-12" style="text-align: left;">Tower Type </label>
                        <div class="col-lg-12">
                          <div class="row">
                            <div class="col-sm-12">
                                <select name="TowerType" class="form-control" id="TowerType">
                                    <option value="">--</option>
                                    <?php while($tower_types=$tower_type->fetch()){ ?>
                                    <option value="<?php echo($tower_types['ID_Tower_Type']) ?>" <?php if(isset($_GET['TowerType']) && $_GET['TowerType']!='' && $tower_types['ID_Tower_Type']==$_GET['TowerType']){echo 'selected'; } ?>><?php echo(stripslashes($tower_types['Design_Tower_Type'])); ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                          </div>
    ...
    $(document).ready(function() {
        ...
        $('#iframe').attr('src', "table_rapport_mensuel_tour.php?User="+$('#utilisateur').val()+'&siteId='+$('#siteId').val()+'&TowerType='+$('#TowerType').val()+'&dateRapport='+dateRap);
    });
  ...
  $(function() {
    ...
    $('#btn_ajouter').click(function(e){
      ...
      let dateRapport = $('#dateRapport').val();
      dateRap = dateRapport.replace(/\//g, "-");
      window.location.replace('ajouter_rapport_mensuel_tour.php?User='+$('#utilisateur').val()+'&siteId='+$('#siteId').val()+'&TowerType='+$('#TowerType').val()+'&dateRapport='+dateRap);
    })
