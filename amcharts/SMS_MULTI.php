    ...
    $response = curl_exec($ch);
     ...
     if ($err) {
        ...
        while ($numeros=$res_numero->fetch()) {
            ...
            $responses = explode('"', $mystring);
            $smsCount = substr($responses[22], 1, 1);
            $update_participer=$pdo->query("UPDATE participer SET Reponse='".$mystring."', smsCount=".$smsCount.", Command=1 WHERE ID_Conversation='".$ID_Conversation."' AND ID_Telephone='".$numeros['ID_Telephone']."'");
        }
    }
    echo "1";
}else{
