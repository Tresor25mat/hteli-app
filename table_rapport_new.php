...
if (isset($_GET['nocTicket']) && $_GET['nocTicket'] != '') {
    $query .= " AND UCASE(table_rapport_journalier_new.Noc_Ticket) LIKE '%" . strtoupper($_GET['nocTicket']) . "%'";
}
    ...
    <?php while ($selections = $selection->fetch()) {
        ...
        ?>
            <tr class="odd gradeX" style="background: transparent;">
                ...
                <td><center>
                    <a href="daily_report_new.php?Rapport=<?php echo ($selections['ID_Rapport']) ?>" title="Imprimer" style="margin-right: 5px; width: 30px; border-radius: 0" class="btn btn-success"><i class="fa fa-print fa-fw"></i></a>
                    <a href="afficher_rapport_new.php?ID=<?php echo ($selections['ID_Rapport']) ?>" title="Afficher" style="margin-right: 5px; width: 30px; border-radius: 0" class="btn btn-info"><i class="fa fa-eye fa-fw"></i></a>
                    <a href="modifier_report_new.php?ID=<?php echo ($selections['ID_Rapport']) ?>&User=<?php if (isset($_GET['User']) && $_GET['User'] != '') {
                          echo $_GET['User'];
                      } ?>&siteId=<?php if (isset($_GET['siteId']) && $_GET['siteId'] != '') {
                           echo $_GET['siteId'];
                       } ?>&nocTicket=<?php if (isset($_GET['nocTicket']) && $_GET['nocTicket'] != '') {
                            echo $_GET['nocTicket'];
                    ...
                    <?php if ($_SESSION['user_eteelo_app']['ID_Statut'] == 1) { ?>
                        <a style="width: 25px; border-radius: 0;" class="btn btn-danger" href="javascript: alertify.confirm('Voulez-vous vraiment supprimer cet enregistrement?\n Toutes les informations concernant cet enregistrement seront supprimées!').set('onok',function(closeEvent){window.location.replace('suppr_rapport_new.php?ID=<?php echo ($selections['ID_Rapport']) ?>&token=<?php echo ($_SESSION['user_eteelo_app']['token']) ?>&User=<?php if (isset($_GET['User']) && $_GET['User'] != '') {
                                echo $_GET['User'];
                            } ?>&siteId=<?php if (isset($_GET['siteId']) && $_GET['siteId'] != '') {
                                 echo $_GET['siteId'];
                             } ?>&nocTicket=<?php if (isset($_GET['nocTicket']) && $_GET['nocTicket'] != '') {
    ...
    <?php } ?>
              ...
              <ul class="pagination ms-auto">
                                ...
                                if ($pagesTotales > 3) {
                                    ...
                                    if ($pageCourante == 1) {
                                        echo '<li class="page-item"><a class="page-link" href="#">1</a></li><li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pageNexte . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pageNexte . '</a></li><li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pageTrois . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pageTrois . '</a></li><li class="page-item"><a class="page-link" href="#">...</a></li>';
                                    } else if ($pageCourante == 2) {
                                        echo '<li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pagePrecedente . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pagePrecedente . '</a></li><li class="page-item active"><a class="page-link" href="#">' . $pageCourante . '</a></li><li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pageNexte . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pageNexte . '</a></li><li class="page-item"><a class="page-link" href="#">...</a></li>';
                                    } else if ($pageCourante == $pagesAvantTotales) {
                                        echo '<li class="page-item"><a class="page-link" href="#">...</a></li><li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pagePrecedente . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pagePrecedente . '</a></li><li class="page-item active"><a class="page-link" href="#">' . $pageCourante . '</a></li><li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pageNexte . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pageNexte . '</a></li>';
                                    } else if ($pageCourante == $pagesTotales) {
                                        echo '<li class="page-item"><a class="page-link" href="#">...</a></li><li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pageAvantPrecedente . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pageAvantPrecedente . '</a></li><li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pagePrecedente . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pagePrecedente . '</a></li><li class="page-item active"><a class="page-link" href="#">' . $pageCourante . '</a></li>';
                                    } else {
                                        echo '<li class="page-item"><a class="page-link" href="#">...</a></li><li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pagePrecedente . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pagePrecedente . '</a></li><li class="page-item active"><a class="page-link" href="#">' . $pageCourante . '</a></li><li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $pageNexte . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $pageNexte . '</a></li><li class="page-item"><a class="page-link" href="#">...</a></li>';
                                    }
                                } else {
                                    for ($i = 1; $i <= $pagesTotales; $i++) {
                                        if ($i == $pageCourante) {
                                            ...
                                            echo '<li class="page-item"><a class="page-link" href="table_rapport_new.php?page=' . $i . '&User=' . $_GET['User'] . '&siteId=' . $_GET['siteId'] . '&nocTicket=' . $_GET['nocTicket'] . '&dateRapport=' . $_GET['dateRapport'] . '">' . $i . '</a></li>';
                                        }
                                    }
                                }
