<?php
    ...
    $query="SELECT * FROM tranche_frais INNER JOIN frais ON tranche_frais.ID_Frais=frais.ID_Frais INNER JOIN taux_change ON frais.ID_Taux=taux_change.ID_Taux WHERE tranche_frais.ID_Tranche_Frais!=0";
    if(isset($_GET['Frais']) && $_GET['Frais']!=''){
        $query.=" AND tranche_frais.ID_Frais=".$_GET['Frais'];
        $frai=$pdo->query("SELECT * FROM frais INNER JOIN taux_change ON frais.ID_Taux=taux_change.ID_Taux WHERE frais.ID_Frais=".$_GET['Frais']);
        $frais=$frai->fetch();
    }
    $query.=" ORDER BY tranche_frais.Design_Tranche_Frais";
    $req=$pdo->query($query);
    $Total=$req->rowCount();
    $totalparpage=10;
                    ...
                    <!-- <div class="panel panel-default"> -->
                        <!-- <div class="panel-body"> -->
                            <table width="100%" class="table-bordered" id="dataTables-example" style="border: 1px solid #E6E7E9">
                                ...
                                <tbody id="MaTable">
    <?php while($selections=$selection->fetch()){$Nbr++;$MontantTotal=$MontantTotal+$selections['Montant_Tranche']; ?>
        <tr style="background: transparent; height: 35px;">
            <td style="width: 80px; "><center><?php echo sprintf('%02d', $Nbr); ?></center></td>
            <td style="padding-left: 10px"><!-- <center> --><?php echo strtoupper(stripslashes($selections['Design_Tranche_Frais'])); ?></td>
            <td style="padding-left: 10px"><!-- <center> --><?php echo number_format($selections['Montant_Tranche'], 2, ',', ' ').stripslashes($selections['Symbole']); ?></td>
            <td><center>
                <?php if($_SESSION['user_eteelo_app']['ID_Statut']==1 || $_SESSION['user_eteelo_app']['ID_Statut']==2){ ?>
                <a style="width: 25px; border-radius: 0;" class="btn btn-danger" href="javascript: alertify.confirm('Voulez-vous vraiment supprimer cet enregistrement?\n Toutes les informations concernant cet enregistrement seront supprimées!').set('onok',function(closeEvent){window.location.replace('suppr_tranche_frais.php?ID=<?php echo($selections['ID_Tranche_Frais']) ?>&token=<?php echo($_SESSION['user_eteelo_app']['token']) ?>&Frais=<?php if(isset($_GET['Frais']) && $_GET['Frais']!=''){echo $_GET['Frais']; } ?>');alertify.success('suppression éffectuée');}).set('oncancel',function(closeEvent){alertify.error('suppression annulée');}).set({title:''},{labels:{ok:'Oui', cancel:'Annuler'}});" title="Supprimer"><i class="fa fa-trash-o fa-fw"></i></a></center>
                <?php } ?>
            </td>
        </tr>
    <?php } ?>
...
<tfoot>
                                    <tr style="height: 35px;">
                                        ...
                                        <td style="font-weight: bold; padding-left: 10px"><?php if(isset($_GET['Frais']) && $_GET['Frais']!=''){ echo number_format($MontantTotal, 2, ',', ' ').stripslashes($frais['Symbole']);} ?></td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
