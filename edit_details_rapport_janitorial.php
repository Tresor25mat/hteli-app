<?php
    ...
    for($i=1; $i<=30; $i++) {
        ...
        if($Token==$_SESSION['user_eteelo_app']['token']){
            $rech=$pdo->query("SELECT * FROM questionnaire_janitorial WHERE ID_Rapport=".$ID_Rapport." AND Indice=".$i);
            if($rechs=$rech->fetch()){
                $rs=$pdo->prepare("UPDATE questionnaire_janitorial SET Test_Results=?, Pass=?, Remarks=? WHERE ID_Rapport=? AND Indice=?");
