    ...
    if(intval($Montant_paye+$Montant_paie)>intval($Rech_frais['Montant_Frais'])){
        ...
        if($Token==$_SESSION['user_eteelo_app']['token']){
            if($Mode_Paiement==1){
                if($ID_Paiement==''){
                    $ID_Paiement=$chaine;
                    $insert_paiement=$pdo->query("INSERT INTO paiement (ID_Paiement, ID_Inscription, Taux, Date_Paiement, ID_Utilisateur) VALUES ('".$ID_Paiement."', '".$eleves['ID_Inscription']."', ".$taux_change['Montant'].", '".$DateP."', ".$_SESSION['user_eteelo_app']['ID_Utilisateur'].")");
                    $rec=$pdo->query("INSERT INTO recu(Num_Recu, ID_Paiement, ID_Utilisateur) VALUES ('".$nombre."', '".$ID_Paiement."', ".$_SESSION['user_eteelo_app']['ID_Utilisateur'].")");
                }
                ...
                $res=$pdo->prepare("INSERT INTO paiement_frais(ID_Paiement, ID_Frais, ID_Operation, ID_Taux, Mode_Paiement, Montant_Paie, ID_Utilisateur) VALUES (?,?,?,?,?,?,?)");
                $param=array($ID_Paiement, $Rech_frais['ID_Frais'], $operations['ID_Operation'], $Devise, $Mode_Paiement, $Montant, $_SESSION['user_eteelo_app']['ID_Utilisateur']);
                $res->execute($param);
                ...
                if($ID_Paiement==''){
                    $ID_Paiement=$chaine;
                    $insert_paiement=$pdo->query("INSERT INTO paiement (ID_Paiement, ID_Inscription, Taux, Date_Paiement, Confirm_Paiement, ID_Utilisateur) VALUES ('".$ID_Paiement."', '".$eleves['ID_Inscription']."', ".$taux_change['Montant'].", '".$DateP."', 2, ".$_SESSION['user_eteelo_app']['ID_Utilisateur'].")");
                    $rec=$pdo->query("INSERT INTO recu(Num_Recu, ID_Paiement, ID_Utilisateur) VALUES ('".$nombre."', '".$ID_Paiement."', ".$_SESSION['user_eteelo_app']['ID_Utilisateur'].")");
                }
                $res=$pdo->prepare("INSERT INTO paiement_frais(ID_Paiement, ID_Frais, ID_Taux, Mode_Paiement, Montant_Paie, ID_Utilisateur) VALUES (?,?,?,?,?,?)");
                $param=array($ID_Paiement, $Rech_frais['ID_Frais'], $Devise, $Mode_Paiement, $Montant, $_SESSION['user_eteelo_app']['ID_Utilisateur']);
                $res->execute($param);
            }
            echo "1,".$ID_Paiement;
        }
