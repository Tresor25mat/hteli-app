    ...
    if (!in_array($extension_img_maj, $extensions_img)) {
        ...
        if (move_uploaded_file($_FILES['fichier_image_' . $i]['tmp_name'], $dossier_image . $Image)) //Si la fonction renvoie TRUE, c'est
        {
            if ($Token == $_SESSION['user_eteelo_app']['token']) {
                $recherche = $pdo->query("SELECT * FROM table_titre_rapport_new WHERE ID_Rapport=" . $ID_Rapport . " AND ID_Sous_Titre=" . $ID_Titre);
                if (!$recherches = $recherche->fetch()) {
                    $insert = $pdo->query("INSERT INTO table_titre_rapport_new SET ID_Rapport=" . $ID_Rapport . ", ID_Sous_Titre=" . $ID_Titre);
                    $selection = $pdo->query("SELECT MAX(ID_Titre_Rapport) AS ID_Titre_Rapport FROM table_titre_rapport_new");
                    $selections = $selection->fetch();
                    $ID_Titre_Rapport = $selections['ID_Titre_Rapport'];
                } else {
                    $ID_Titre_Rapport = $recherches['ID_Titre_Rapport'];
                }
                $rech = $pdo->query("SELECT * FROM table_photo_rapport_new WHERE ID_Titre_Rapport=" . $ID_Titre_Rapport . " AND Indice_Photo=" . $i);
                if ($rechs = $rech->fetch()) {
                    $rs = $pdo->prepare("UPDATE table_photo_rapport_new SET Photo=? WHERE ID_Titre_Rapport=? AND Indice_Photo=?");
                    $params = array($Image, $ID_Titre_Rapport, $i);
                    $rs->execute($params);
                } else {
                    $rs = $pdo->prepare("INSERT INTO table_photo_rapport_new SET ID_Titre_Rapport=?, Indice_Photo=?, Photo=?");
