...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed" style="overflow-x: hidden;">
            ...
            <div class="row g-2 align-items-center">
              ...
              <div class="col-12">
                <div class="row" style="border-bottom: 1px solid #EEEEEE; padding-bottom: 20px">
                    ...
                    <div class="col-md-<?php if($_SESSION['user_eteelo_app']['ID_Statut']==3){ echo '2';}else{echo '1';} ?>">
                      <div class="form-group ">
                        <label for="RectifierMake" class="control-label col-lg-12" style="text-align: left;">Rectif. make </label>
                        <div class="col-lg-12">
                          <div class="row">
                            <div class="col-sm-12">
                                <select name="RectifierMake" class="form-control" id="RectifierMake">
                                    <option value="">--</option>
                                    <?php while($rectifier_makes=$rectifier_make->fetch()){ ?>
                                    <option value="<?php echo($rectifier_makes['ID_Rectifier_Make']) ?>" <?php if(isset($_GET['RectifierMake']) && $_GET['RectifierMake']!='' && $rectifier_makes['ID_Rectifier_Make']==$_GET['RectifierMake']){echo 'selected'; } ?>><?php echo(stripslashes($rectifier_makes['Design_Rectifier_Make'])); ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                          </div>
    ...
    $(document).ready(function() {
        ...
        $('#iframe').attr('src', "table_rapport_rectifier.php?User="+$('#utilisateur').val()+'&siteId='+$('#siteId').val()+'&RectifierMake='+$('#RectifierMake').val()+'&dateRapport='+dateRap);
    });
    ...
    $('#RectifierMake').change(function(){
        if($('#RectifierMake').val()!=''){
            $('#btn_afficher').focus();
        }
    });
  ...
  $(function() {
    ...
    $('#btn_ajouter').click(function(e){
      ...
      let dateRapport = $('#dateRapport').val();
      dateRap = dateRapport.replace(/\//g, "-");
      window.location.replace('ajouter_rapport_rectifier.php?User='+$('#utilisateur').val()+'&siteId='+$('#siteId').val()+'&RectifierMake='+$('#RectifierMake').val()+'&dateRapport='+dateRap);
    })
