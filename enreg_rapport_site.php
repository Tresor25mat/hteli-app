        ...
        if(!in_array($extension_maj, $extensions))
        {
            ...
            if(move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier_fichier . $Fichier)) //Si la fonction renvoie TRUE, c'est
            {
                if($Token==$_SESSION['user_eteelo_app']['token']){
                    $rs=$pdo->prepare("INSERT INTO table_rapport_mensuel_site (ID_Site, Date_Rapport, Num_Ref, Time_In, Time_Out, Fichier, ID_Utilisateur) VALUES (?,?,?,?,?,?,?)");
                    $params=array($Site, $Daterap, $Num_Ref, $Time_in, $Time_out, $Fichier, $_SESSION['user_eteelo_app']['ID_Utilisateur']);
                    $rs->execute($params);
