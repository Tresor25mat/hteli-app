...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed" style="overflow-x: hidden;">
              ...
              <div class="col-12">
                <div class="row" style="border-bottom: 1px solid #EEEEEE; padding-bottom: 20px">
                    ...
                    <div class="col-12 col-md-auto ms-auto d-print-none" style="margin-top: 18px">
                        <div class="btn-list">
                        <a href="#" id="btn_ajouter" class="btn btn-primary d-sm-inline-block" title="Ajouter une devise">
    ...
    $(document).ready(function() {
        ...
        $('#iframe').attr('src', "table_devise.php?Ecole="+$('#ecole').val());
    });
  ...
  $(function() {
    ...
    $('#enregistrer').click(function(){
        if($('#liste_devise').val()=='' || $('#liste_ecole').val()=='' || $('#taux_devise').val()==''){
                alertify.alert('<?php echo $app_infos['Design_App']; ?>','Veuillez remplir tous les champs obligatoires svp!');
                $('#liste_devise').focus();
        }else{
                $.ajax({
                        url:'enreg_devise.php',
                        type:'post',
                        beforeSend:function(){
                        },
                        dataType:'text',
                        data: {Devise:$('#liste_devise').val(), Ecole:$('#liste_ecole').val(), Montant:$('#taux_devise').val(), token:$('#tok').val()},
                        success:function(ret){
                            if(ret==1){
