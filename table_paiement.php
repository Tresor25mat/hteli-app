<?php
    ...
    if(isset($_GET['Recu']) && $_GET['Recu']!=''){
        $query.=" AND recu.Num_Recu ='".$_GET['Recu']."'";
    }
    ...
    <?php while($selections=$selection->fetch()){$Nbr++; ?>
        <tr class="odd gradeX" style="background: transparent;">
            ...
            <a href="recu.php?Paiement=<?php echo($selections['ID_Paiement']) ?>&token=<?php echo($_SESSION['user_eteelo_app']['token']) ?>&Ecole=<?php if(isset($_GET['Ecole']) && $_GET['Ecole']!=''){echo $_GET['Ecole']; } ?>" title="Afficher les détails" style="margin-right: 5px; border-radius: 0; width: 30px" class="btn btn-success"><i class="fa fa-print fa-fw"></i></a>
                ...
                <a style="width: 25px; border-radius: 0;" class="btn btn-danger" href="javascript: alertify.confirm('Voulez-vous vraiment supprimer cet enregistrement?\n Toutes les informations concernant cet enregistrement seront supprimées!').set('onok',function(closeEvent){window.location.replace('suppr_paiement.php?ID=<?php echo($selections['ID_Paiement']) ?>&token=<?php echo($_SESSION['user_eteelo_app']['token']) ?>&Ecole=<?php if(isset($_GET['Ecole']) && $_GET['Ecole']!=''){echo $_GET['Ecole']; } ?>&Annee=<?php if(isset($_GET['Annee']) && $_GET['Annee']!=''){echo $_GET['Annee']; } ?>&Classe=<?php if(isset($_GET['Classe']) && $_GET['Classe']!=''){echo $_GET['Classe']; } ?>&Eleve=<?php if(isset($_GET['Eleve']) && $_GET['Eleve']!=''){echo $_GET['Eleve']; } ?>&Recu=<?php if(isset($_GET['Recu']) && $_GET['Recu']!=''){echo $_GET['Recu']; } ?>');alertify.success('suppression éffectuée');}).set('oncancel',function(closeEvent){alertify.error('suppression annulée');}).set({title:''},{labels:{ok:'Oui', cancel:'Annuler'}});" title="Supprimer"><i class="fa fa-trash-o fa-fw"></i></a></center>
                <?php } ?>
            </td>
        </tr>
    <?php } ?>
              ...
              <ul class="pagination ms-auto">
                                ...
                                if($pagesTotales>3){
                                    ...
                                    if($pageCourante==1){
                                        echo '<li class="page-item"><a class="page-link" href="#">1</a></li><li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pageNexte.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pageNexte.'</a></li><li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pageTrois.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pageTrois.'</a></li><li class="page-item"><a class="page-link" href="#">...</a></li>';
                                    }else if($pageCourante==2){
                                        echo '<li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pagePrecedente.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pagePrecedente.'</a></li><li class="page-item active"><a class="page-link" href="#">'.$pageCourante.'</a></li><li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pageNexte.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pageNexte.'</a></li><li class="page-item"><a class="page-link" href="#">...</a></li>';
                                    }else if($pageCourante==$pagesAvantTotales){
                                        echo '<li class="page-item"><a class="page-link" href="#">...</a></li><li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pagePrecedente.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pagePrecedente.'</a></li><li class="page-item active"><a class="page-link" href="#">'.$pageCourante.'</a></li><li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pageNexte.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pageNexte.'</a></li>';
                                    }else if($pageCourante==$pagesTotales){
                                            echo '<li class="page-item"><a class="page-link" href="#">...</a></li><li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pageAvantPrecedente.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pageAvantPrecedente.'</a></li><li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pagePrecedente.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pagePrecedente.'</a></li><li class="page-item active"><a class="page-link" href="#">'.$pageCourante.'</a></li>';
                                    }else{
                                        echo '<li class="page-item"><a class="page-link" href="#">...</a></li><li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pagePrecedente.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pagePrecedente.'</a></li><li class="page-item active"><a class="page-link" href="#">'.$pageCourante.'</a></li><li class="page-item"><a class="page-link" href="table_paiement.php?page='.$pageNexte.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$pageNexte.'</a></li><li class="page-item"><a class="page-link" href="#">...</a></li>'; 
                                    }
                                }else{
                                    for ($i=1; $i <= $pagesTotales ; $i++) { 
                                        if ($i==$pageCourante) {
                                            ...
                                            echo '<li class="page-item"><a class="page-link" href="table_paiement.php?page='.$i.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">'.$i.'</a></li>';
                                        }
                                    } 
                                }
                                if($pagesTotales>$pageCourante){
                                    $page=$pageCourante+1;
                                    echo '<li class="page-item"><a class="page-link" href="table_paiement.php?page='.$page.'&Ecole='.$_GET['Ecole'].'&Annee='.$_GET['Annee'].'&Classe='.$_GET['Classe'].'&Eleve='.$_GET['Eleve'].'&Recu='.$_GET['Recu'].'">Next<svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><polyline points="9 6 15 12 9 18" /></svg></a></li>';
                                }else{
