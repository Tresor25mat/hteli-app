<?php
    ...
    if($Token==$_SESSION['user_eteelo_app']['token']){
        $rs=$pdo->prepare("UPDATE table_rapport_rectifier SET ID_Site=?, ID_Rectifier_Make=?, ID_Rectifier_Model=?, ID_Make_Batterie=?, Number_Bat_By_bank=?, Date_Rapport=?, Num_Work_Order=?, History_Card_Ref=?, Num_Serial=?, Rectifier_Capacity=?, Number_Bat_banks=?, Time_In=?, Time_Out=?, Updated_By=? WHERE ID_Rapport=?");
        $params=array($Site, $Rectifier_Make, $Rectifier_Model, $Batterie_Make, $Num_Bat, $Daterap, $Num_Work_Order, $History_Card_Ref, $Num_Serial, $Rectifier_Capacity, $Number_Bat, $Time_in, $Time_out, $_SESSION['user_eteelo_app']['ID_Utilisateur'], $ID_Rapport);
        $rs->execute($params);
    }
?>